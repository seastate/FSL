...
class Consumer():
    ...
    """
    def __init__(self,m=0,pars=None,mesh=None,verbose=False,color='b'):
        ...
        self.color = color
        # Create the cell variable instance
        self.Z = CellVariable(name="consumer1",mesh=mesh, value=0., hasOld=1)
        # set parameters, if provided
        if pars:
            self.setpars(pars=pars,init=True)
...
class FSL1Dsim():
    ...
    def setup(self):
        ...
        self.Cons = [Consumer(mesh=self.mesh) for i in range(self.M)]
